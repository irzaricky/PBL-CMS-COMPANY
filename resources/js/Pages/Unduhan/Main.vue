    ...
    <AppLayout>
        <div
                ...
                <div class="w-full overflow-x-auto">
                    <div class="flex gap-2 whitespace-nowrap py-1">
                        ...
                        <template v-if="isLoading">
                            <div v-for="n in 4" :key="n" class="px-4 py-2 rounded-xl bg-gray-200 animate-pulse w-24 h-9"></div>
                        </template>
            ...
            <div class="w-full flex flex-col gap-12">
                <!-- Loading skeleton -->
                <template v-if="isLoading">
                    <div v-for="n in 3" :key="`skeleton-${n}`" class="flex flex-col lg:flex-row justify-between gap-16 animate-pulse">
                        <div class="flex flex-col lg:flex-row gap-8 items-start lg:items-center w-full">
                            <div class="rounded-2xl bg-gray-200 aspect-video w-full lg:w-[296px] h-[166px]"></div>
                ...
                <!-- Empty state -->
                <div v-else-if="unduhanList.length === 0" class="flex flex-col items-center justify-center gap-6 py-20 text-center">
                    <div class="flex flex-col lg:flex-row items-center gap-6 text-left">
                        <img src="/image/empty.svg" alt="Empty State" class="w-40 h-40 lg:w-96 lg:h-96 object-contain" />
                        <div>
                            <h3 class="text-xl md:text-2xl font-semibold text-gray-700">Yah, file tidak ditemukan</h3>
                            <p class="text-sm md:text-base text-gray-500">Coba kata kunci lain atau pilih kategori berbeda.</p>
                        </div>
                    </div>
                </div>
