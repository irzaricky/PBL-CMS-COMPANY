...
<template>
    <div
        ...
        <div class="w-full max-w-2xl flex flex-col gap-4 items-center text-center">
            <div class="text-typography-dark text-base font-semibold font-custom">We Are</div>
			<div class="text-typography-dark text-5xl font-bold leading-[1.2]">Open Positions</div>
        	<div class="text-typography-dark text-lg font-normal leading-relaxed">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros elementum tristique.</div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row w-full max-w-7xl gap-12">
            ...
            <div class="flex-1 flex flex-col gap-12">
                <div v-for="lowongan in lowongan" :key="lowongan.id"
                    class="border-t border-typography-hover2 pt-8 flex flex-col gap-6">
                    <!-- Title & Department -->
                    <div class="flex flex-wrap items-center gap-4">
                        <div class="text-typography-dark text-2xl font-bold">{{ lowongan.judul_lowongan }}</div>
                        <span
                            class="px-2.5 border border-typography-hover2 py-1 rounded-full text-base font-semibold text-typography-dark">
                            {{ lowongan.jenis_lowongan }}
                        </span>
                    </div>

                    <!-- Deskripsi -->
                    <p class="text-typography-dark text-base font-normal">
                    ...
                    <div class="flex flex-wrap gap-6">
                        <div class="flex items-center gap-3">
                            <AlarmClock class="w-5 h-5 text-typography-dark" />
                            <span class="text-base font-semibold text-typography-dark">Ditutup pada {{ formatTanggal(lowongan.tanggal_ditutup)
                                }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <Wallet class="w-5 h-5 text-typography-dark" />
                            <span class="text-base font-semibold text-typography-dark">Rp.{{ formatGaji(lowongan.gaji) }}</span>
                        </div>
                    </div>

                    <!-- Tombol Apply -->
                    <a :href="`/lowongan/${lowongan.slug}`"
                        class="inline-block w-fit px-5 py-2 bg-secondary rounded-full text-base font-medium text-primary hover:bg-secondary/50">
                        Apply Now
                    </a>
                </div>
            </div>
