...
const dynamicStats = computed(() => {
    const uniqueMitra = [...new Set(caseStudies.value.map(cs => cs.mitra?.nama_mitra).filter(Boolean))];
    const currentYear = new Date().getFullYear();
    const thisYearCases = caseStudies.value.filter(cs => 
        new Date(cs.created_at).getFullYear() === currentYear
    );
    
    return {
        totalCases: caseStudies.value.length,
        uniquePartners: uniqueMitra.length,
                ...
                <div class="w-full max-w-2xl flex flex-col items-center gap-4">
                    <div class="w-full relative">
                        <input v-model="searchQuery" @input="handleSearch" type="text"
                            placeholder="Cari studi kasus berdasarkan industri, tantangan, atau solusi..." class="w-full px-6 py-4 rounded-full bg-white/10 backdrop-blur-sm text-white 
        ...
        <div class="max-w-screen-xl mx-auto px-4 lg:px-8 py-16 flex flex-col items-center gap-20 overflow-hidden">
            ...
            <section v-if="featuredCase" class="w-full">
                <div class="w-full mb-12 flex flex-col gap-4">
                    <div class="text-secondary text-base font-semibold font-custom uppercase tracking-wider">Studi Kasus
                        Unggulan</div>
                ...
                <div
                    ...
                    <div class="flex flex-col justify-center p-8 lg:p-12 gap-6">
                        ...
                        <h3 class="text-2xl lg:text-4xl font-custom font-normal">{{ featuredCase.judul_case_study }}
                        </h3>
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            <div class="flex items-center gap-2 text-gray-700">
                                ...
                                <span class="font-custom">{{ formatDate(featuredCase.created_at) }}</span>
                            </div>
                        </div>
                        <Link :href="`/case-study/${featuredCase.slug_case_study}`" class="mt-6 inline-flex items-center justify-center gap-2 
                            px-6 py-3 bg-secondary text-white font-medium rounded-full 
                            hover:bg-black transition-all duration-300 w-fit font-custom">
                        Baca Studi Kasus
            ...
            <div class="w-full">
                ...
                <div v-if="!isLoading && caseStudies.length > 0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div v-for="item in caseStudies" :key="item.case_study_id"
                        ...
                        <div class="p-6 flex flex-col gap-4">
                            ...
                            <Link :href="`/case-study/${item.slug_case_study}`" class="group-hover:text-secondary">
                            ...
                            <div class="mt-2 text-sm text-gray-500 font-custom flex items-center gap-2">
                                <Calendar class="w-4 h-4" />
                                {{ formatDate(item.created_at) }}
                            </div>

                            <Link :href="`/case-study/${item.slug_case_study}`"
                ...
                <div v-if="!isLoading && lastPage > 1" class="flex justify-center items-center gap-4 mt-16 font-custom">
                    ...
                    <div class="flex gap-2">
                        <button v-for="page in Math.min(3, lastPage)" :key="page" @click="goToPage(page)"
                        ...
                        <button v-if="lastPage > 3" @click="goToPage(lastPage)"
                            class="w-10 h-10 rounded-full flex items-center justify-center font-medium transition"
            ...
            <section
                ...
                <div class="relative z-10 max-w-4xl mx-auto flex flex-col items-center gap-8">
                    <!-- Icon -->
                    <div class="w-72 h-72 bg-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                        <img src="/image/mitra.svg" alt="Success"
                            class="object-contain" />
                    </div>

                    <!-- Heading -->
                    <div class="text-center">
                        <h2 class="text-3xl lg:text-5xl font-normal font-custom text-white leading-tight mb-4">
                            Siap untuk membuat kisah sukses Anda sendiri?
                        </h2>
                        <p class="text-lg lg:text-xl text-white/80 font-custom max-w-2xl mx-auto">
                            Mari bekerja sama untuk mengubah tantangan bisnis Anda menjadi kisah sukses yang terukur dan
                            menginspirasi.
                        </p>
                    </div>

                    <!-- Stats -->
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-2xl">
                        <div class="text-center">
                            <div class="text-2xl lg:text-3xl font-bold text-white font-custom">{{ dynamicStats.totalCases }}+</div>
                            <div class="text-sm text-white/70 font-custom">Studi Kasus</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl lg:text-3xl font-bold text-white font-custom">{{ dynamicStats.uniquePartners }}+</div>
                            <div class="text-sm text-white/70 font-custom">Mitra Terpercaya</div>
                        </div>
                        <div class="text-center col-span-2 lg:col-span-1">
                            <div class="text-2xl lg:text-3xl font-bold text-white font-custom">{{ dynamicStats.thisYearCases }}+</div>
                            <div class="text-sm text-white/70 font-custom">Proyek Tahun Ini</div>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex flex-col sm:flex-row gap-4 mt-4 font-custom">
                        <Link href="/kontak"
                    ...
                    <!-- Trust Badge -->
                    <div class="mt-8 flex items-center justify-center gap-2 text-white/60 text-sm font-custom">
                        <div class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                        <span>Konsultasi gratis untuk proyek pertama Anda</span>
                    </div>
                </div>
            </section>
        </div>
