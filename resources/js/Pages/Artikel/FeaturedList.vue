                    ...
                    <div class="relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-8 text-white">
                            <div class="flex items-center gap-4 mb-4">
                                <span class="px-3 py-1.5 bg-secondary/90 backdrop-blur-sm text-white text-sm font-semibold rounded-full border border-white/20">
                                    {{ featuredArticles[0].kategoriArtikel?.nama_kategori_artikel || 'Tanpa Kategori' }}
                                </span>
                                <span class="flex items-center gap-2 text-sm font-medium bg-black/30 px-3 py-1.5 rounded-full backdrop-blur-sm">
                            ...
                            <a :href="`/artikel/${featuredArticles[0].slug}`" class="group/link">
                                <h3 class="text-2xl lg:text-3xl font-bold mb-3 group-hover/link:text-white transition-colors duration-300">
                    ...
                    <div class="space-y-6">
                        <div v-for="(post, i) in featuredArticles.slice(1, 4)" :key="i"
                            ...
                            <img class="w-full h-64 object-cover group-hover:scale-105 transition-transform duration-500"
                                :src="getImageUrl(post.thumbnail_artikel)" 
                                alt="thumbnail" />
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-5">
                                <div class="flex items-center gap-3 mb-3">
                                    <span class="px-2.5 py-1 bg-secondary/80 backdrop-blur-sm text-white text-xs font-semibold rounded-full border border-white/20">
                                        {{ post.kategoriArtikel?.nama_kategori_artikel || 'Tanpa Kategori' }}
                                    </span>
                                    <span class="flex items-center gap-1.5 text-xs font-medium bg-black/40 px-2.5 py-1 rounded-full backdrop-blur-sm text-white">
                                ...
                                <a :href="`/artikel/${post.slug}`" class="block group/title mb-2">
                                    <h3 class="text-lg font-bold text-white group-hover/title:text-gray-200 transition-colors duration-300 line-clamp-2">
                                        {{ post.judul_artikel }}
                                    </h3>
                                </a>
